...
def getdata():
    ...
    with st.empty():
        ...
        for bticker in bsymbols[:,5]:
            ...
            index += 1
            df=yf.download(bticker,period="2y",interval="60m")
            ...
            df5w= df4.resample('D').agg(ohlcv_dict)
            df6w= df5w.dropna(inplace=True)
            df6w.to_sql(bticker,enginew, if_exists='replace')
        ...
        #and frame['RISK'].iloc[-1]<=riskvalue:
            ...
            #or framew['Close'].iloc[-1]>framew['sup2'].iloc[-1] or framew['Close'].iloc[-1]>framew['sup4'].iloc[-1])):
                ...
                if option2 == 'pullback':  
                   ...
                   and frame['Dec_EMA200'].iloc[-h1]=='Buy'\
                   and frame['Close'].iloc[-h1]>frame['Open'].iloc[-h1]:
