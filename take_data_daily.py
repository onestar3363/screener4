...
def EMA_decision(df):
    ...
    df.loc[(df.Close.shift(1)>=df.EMA20.shift(1))&(df.Low<=df.EMA20)&(df.Close.shift(1)>df.Close), 'EMA20_cross'] = 'Buy2'
    ...
    df.loc[(df.Close.shift(1)>=df.EMA50.shift(1))&(df.Low<=df.EMA50)&(df.Close.shift(1)>df.Close), 'EMA50_cross'] = 'Buy2'
...
def expander(cond):
       with st.expander(cond+' '+str(sira) +') '+ name+'/'+' RISK= '+str(frame['RISK'].iloc[-1].round(2))+'/ %ATR='+str(frame['ATR%'].iloc[-1].round(2))+'/ %wATR='+str(framew['ATR%'].iloc[-1].round(2))+str(frame['ATR%'].iloc[-1].round(2))+'/ %wATR='+str(frameh['ATR%'].iloc[-1].round(2))):
           ...
           col4.write(frame[['Close','ATR%','ADX','Dec_EMA50','Dec_MACD','Trend MACD','MACD_diff']].tail(2))
